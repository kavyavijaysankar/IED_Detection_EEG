...
from detect_data import ELECTRODES, load_recording_path
...
def main():
    ...
    if bads['absent']:
        print(f"  note: {name(bads['absent'])} not in this file — left out, not reconstructed")
    if bads['soft']:
    ...
    for e in events:
        print(f"  t={e['time'] / pipe.cfg.sfreq:6.2f}s  ch={ELECTRODES[e['channel']]:5s}  "
