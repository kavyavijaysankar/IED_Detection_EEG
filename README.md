# IED Detection in EEG

Detects interictal epileptiform discharges (IEDs) in scalp EEG recordings, and separately classifies
IED vs. non-IED transients. The detector runs a staged pipeline (preprocessing → candidate generation →
event assembly → classification → decision) over EDF recordings and scores each detected event.
...
- `detect_config.py` - `Config` — every tunable
- `detect_data.py` - preprocessing, channel matching, electrode positions, train/test split
- `detect_stage1.py` - L1 primitives
- `detection.py` - L2, L3, L4 and `DetectionPipeline`
- `detect_metrics.py` - FROC, localisation, cross-validation — evaluation only
- `detect_plots.py` - one function per figure panel

### Notebooks

- `IED classification/` - EDA, classification (registered / unregistered), Fourier and nested-CV experiments
- `IED Detection/` - detection pipeline runs, stage-1 diagnostics, recording-level evaluation

### Scripts

- `predict.py` - scan one EDF with a saved model, print scored events
- `pilot.py` - scan a folder, write per-recording and per-event CSVs

### `detect_model.joblib`

A fitted `DetectionPipeline` (`Config` + classifier + L4), saved with `joblib.dump`/`joblib.load`
(`src/detection.py`). Bundling all three into one file means a recording is always scored with the exact
preprocessing and thresholds it was trained under.
...
Scan a single EDF with the saved model:

```
python predict.py detect_model.joblib path/to/recording.edf
```

Scan a folder of recordings, writing per-recording and per-event CSVs to `results/`:

```
python pilot.py --dir path/to/folder
```

`pilot.py` also takes `--model` (default: `detect_model.joblib` beside the script), `--threshold`,
`--out`, `--check` (validate headers/montage only, then stop), `--resume`, and `--selfcheck` (end-to-end
test on the Kural recordings).

Each `src/` module runs its own self-check with `python src/<module>.py`.

## The pipeline

Wat each stage of the detector does.
